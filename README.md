<div align="center">

# Jainam Doshi
### AI Engineer • Backend Engineer • Software Engineer

<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=22&pause=900&color=22C55E&center=true&vCenter=true&width=760&lines=Building+AI-driven+systems;Production-first+ML+%26+Computer+Vision;Scalable+Backend+Engineering;Document+Understanding+%7C+Blueprint+Reconstruction" />

<p><b>Mumbai, India</b></p>

<p>
Software engineer focused on building AI-driven systems and scalable backend platforms.<br/>
My work sits at the intersection of <b>machine learning</b>, <b>computer vision</b>, and <b>system architecture</b>, with a strong focus on building real production-oriented pipelines.
</p>

<p>
Currently pursuing a <b>6-month internship</b> while developing projects involving <b>AI-powered document understanding</b> and
<b>spatial reconstruction from architectural blueprints</b>.
</p>

<!-- CTA Buttons -->
<p>
  <a href="https://linkedin.com/in/Doshi-Jainam" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="mailto:jainamdoshi462@gmail.com">
    <img src="https://img.shields.io/badge/Email-Contact-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
  <a href="https://github.com/DarkPheonix444?tab=repositories" target="_blank">
    <img src="https://img.shields.io/badge/Projects-View-111827?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
</p>

<!-- Banner -->
<img src="https://capsule-render.vercel.app/api?type=rounded&color=22c55e&height=110&section=header&text=AI%20%7C%20Backend%20%7C%20Computer%20Vision&fontColor=0b1220&fontSize=24" />

</div>

---

## Featured Projects
> Add your real repo links here (recommended). If you tell me the repo names/URLs, I’ll wire them in cleanly.

- **Resume Analysis Engine** — _NLP + Backend_  
  Repo: `https://github.com/DarkPheonix444/resumeproject` 

- **2D → 3D Blueprint Reconstruction System** — _Computer Vision + Three.js_  
  Repo: `https://github.com/DarkPheonix444/2d-to-3d` 

---

## Projects

<details open>
<summary><b>🧾 Resume Analysis Engine</b></summary>

An AI-powered system that analyzes resumes, extracts structured information, and evaluates candidate skills against job descriptions.

**Key features**
- Resume parsing and structured data extraction
- Skill detection and normalization
- Job description matching engine
- Semantic scoring and evaluation pipeline

**Tech stack:** Python, Django, NLP, embeddings, PostgreSQL

</details>

<details open>
<summary><b>🏗️ 2D → 3D Blueprint Reconstruction System</b></summary>

A computer vision pipeline that converts architectural floor plans into structured layouts that can later generate interactive 3D walkthrough environments.

**Pipeline components**
- Image preprocessing and normalization
- Wall and intersection detection
- Layout graph construction
- Room boundary reconstruction
- 3D model generation pipeline

**Tech stack:** Python, OpenCV, PyTorch, Three.js

</details>

---

## Tech Stack

### Languages
![Python](https://img.shields.io/badge/Python-111827?style=for-the-badge&logo=python&logoColor=F7DF1E)
![JavaScript](https://img.shields.io/badge/JavaScript-111827?style=for-the-badge&logo=javascript&logoColor=F7DF1E)

### Backend
![Django](https://img.shields.io/badge/Django-0B3B2E?style=for-the-badge&logo=django&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-111827?style=for-the-badge&logo=flask&logoColor=white)

### AI / Machine Learning
![PyTorch](https://img.shields.io/badge/PyTorch-111827?style=for-the-badge&logo=pytorch&logoColor=EE4C2C)
![OpenCV](https://img.shields.io/badge/OpenCV-111827?style=for-the-badge&logo=opencv&logoColor=5C3EE8)
![scikit-learn](https://img.shields.io/badge/scikit--learn-111827?style=for-the-badge&logo=scikitlearn&logoColor=F7931E)

### Frontend
![React](https://img.shields.io/badge/React-111827?style=for-the-badge&logo=react&logoColor=61DAFB)
![Three.js](https://img.shields.io/badge/Three.js-111827?style=for-the-badge&logo=three.js&logoColor=white)

### Databases & Tools
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-111827?style=for-the-badge&logo=postgresql&logoColor=4169E1)
![MongoDB](https://img.shields.io/badge/MongoDB-111827?style=for-the-badge&logo=mongodb&logoColor=47A248)
![Docker](https://img.shields.io/badge/Docker-111827?style=for-the-badge&logo=docker&logoColor=2496ED)
![Git](https://img.shields.io/badge/Git-111827?style=for-the-badge&logo=git&logoColor=F05032)

---

## Research Interests
- Computer vision systems
- Spatial understanding from 2D architectural layouts
- AI-driven document analysis
- Open-source hardware development (3D printers)

---

## GitHub Stats
<div align="center">

<!-- Overall stats -->
<img height="170" src="https://github-readme-stats.vercel.app/api?username=DarkPheonix444&show_icons=true&theme=tokyonight&hide_border=true" />

<!-- Streak -->
<img height="170" src="https://streak-stats.demolab.com?user=DarkPheonix444&theme=tokyonight&hide_border=true" />

<!-- Top languages -->
<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=DarkPheonix444&layout=compact&theme=tokyonight&hide_border=true" />

</div>

---

## Connect
- LinkedIn: https://linkedin.com/in/Doshi-Jainam
- Email: [jainamdoshi462@gmail.com](mailto:jainamdoshi462@gmail.com)

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=22c55e&height=120&section=footer"/>
</div>
